...
/**********************************************************
  ...
  *
  *********************************************************/
unsigned int bq24158_read_interface (unsigned char RegNum, unsigned char *val, unsigned char MASK, unsigned char SHIFT)
{
   ...
   battery_log(BAT_LOG_FULL,"--------------------------------------------------\n");
	...
	battery_log(BAT_LOG_FULL,"[bq24158_read_interface] Reg[%x]=0x%x\n", RegNum, bq24158_reg);
...
unsigned int bq24158_config_interface (unsigned char RegNum, unsigned char val, unsigned char MASK, unsigned char SHIFT)
{
    unsigned char bq24158_reg = 0;
    int ret = 0;
    ...
    battery_log(BAT_LOG_FULL,"[bq24158_config_interface] write Reg[%x]=0x%x\n", RegNum, bq24158_reg);
...
void bq24158_hw_init(void)
{
    if(g_enable_high_vbat_spec == 1)
    {
        if(g_pmic_cid == 0x1020)
        {
            ...
            bq24158_reg_config_interface(0x06,0x70); // set ISAFE
        }
        else
        {
